...
if __name__ == '__main__':
    ...
    def extract(full_string, message_history):
        # todo: add error handling
        # extract last generated string from whole
    ...
    def function_caller(function_call, functions):
        ...
        # }
        for f in functions:
            if f['name'] == function_call['name'][0]:
                function = f
                break
        else:
            raise ValueError("function not in functions")
        return function['eval_args']

    if "function_call" in extracted:
        print(function_caller(extracted['function_call'], functions))